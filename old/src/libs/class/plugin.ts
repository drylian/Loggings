...
export interface LoggingsPluginData<Defaults> {
     ...
     */
    onCreateInstance?<
        InstanceConfigs extends {} extends LoggingsPluginData<object>
            ? InstanceConfigs
            : LoggingsPluginData<object>,
    >(
        instance: LoggingsCORE<InstanceConfigs>,
    ): unknown;
     ...
     * @returns {unknown} - The result of the function execution.
     */
    onAddPlugin?<PluginData>(
        plugin: LoggingsPluginData<PluginData>,
    ): unknown;
 ...
 */
export function LoggingsPlugin<Defaults>(
    opts: LoggingsPluginData<Defaults>,
) {
    const empty = () => {};
    const plugin = {
        ...opts,
        ...
        onMessage: opts.onMessage ?? empty,
        onCreateInstance: opts.onCreateInstance ?? empty,
        onAddPlugin: opts.onAddPlugin ?? empty,
        onRemPlugin: opts.onRemPlugin ?? empty,
    };
    return plugin;
}
